...
This project provides a Docker container for [ImgBurn](https://www.imgburn.com).
...
> [!NOTE]
> This Docker container is entirely unofficial and not made by the creators of ImgBurn.
...
services:
  imgburn:
    image: jlesage/imgburn
    ports:
      - "5800:5800"
    volumes:
      - "/docker/appdata/imgburn:/config:rw"
...
[Releases]: https://github.com/jlesage/docker-imgburn/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/imgburn/tags
...
container, it would proxy all HTTP requests for `imgburn.domain.tld` to
...
server {
	...
	server_name imgburn.domain.tld;

	location / {
	        proxy_pass http://docker-imgburn;
	}
...
server {
	...
	location = /imgburn {return 301 $scheme://$http_host/imgburn/;}
	location /imgburn/ {
		proxy_pass http://docker-imgburn/;
		# Uncomment the following line if your Nginx server runs on a port that
		# differs from the one seen by external clients.
		#port_in_redirect off;
		location /imgburn/websockify {
			proxy_pass http://docker-imgburn/websockify;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
		...
		# Needed when audio support is enabled.
		location /imgburn/websockify-audio {
			proxy_pass http://docker-imgburn/websockify-audio;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of ImgBurn integrated into Docker image. |
...
Any desktop notification generated by ImgBurn is forwarded to the
...
second, etc). To allow ImgBurn to access the first drive, use
this parameter:
